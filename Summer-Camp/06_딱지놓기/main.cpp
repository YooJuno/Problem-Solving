...
extern int put(int mID, int mRow, int mCol, int mHeight, int mWidth);
extern int getCount(int mID);
...
static bool run()
{
    ...
    for (int q = 0; q < query_num; q++)
    {
        ...
        else if (query == CMD_PUT)
        {
            ...
            int ret = put(mID, mRow, mCol, mHeight, mWidth);
            scanf("%d", &ans);
            if (ans != ret)
            {
                ok = false;
